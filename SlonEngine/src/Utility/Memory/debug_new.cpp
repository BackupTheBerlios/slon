...
#include "Utility/Memory/fast_mutex.h"
#include "Utility/Memory/static_assert.h"
 ...
 */
static void* alloc_mem(size_t size, const char* file, int line, bool is_array, int align = sizeof(int))
{
    assert(line >= 0);
    ...
    new_ptr_list_t* ptr = (new_ptr_list_t*)slon::align_alloc(s, align);
    if (ptr == NULL)
    {
...
void* debug_aligned_alloc(size_t size, int align)
{
    return alloc_mem(size, (char*)_DEBUG_NEW_CALLER_ADDRESS, 0, false, align);
}
...
void debug_aligned_free(void* ptr)
{
