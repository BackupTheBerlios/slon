...
void exportConstraint()
{
    ...
    class_<Constraint::DESC, boost::shared_ptr<Constraint::DESC>, boost::noncopyable>("DESC")
        ...
        .add_property("rigidBodyA",         make_function(DESCgetRigidBodyA, with_custodian_and_ward<1, 2>()),      DESCsetRigidBodyA)
        .add_property("rigidBodyB",         make_function(DESCgetRigidBodyB, with_custodian_and_ward<1, 2>()),      DESCsetRigidBodyB)
        .add_property("frameA",             make_function(DESCgetFrameA, with_custodian_and_ward<1, 2>()),          DESCsetFrameA)
        .add_property("frameB",             make_function(DESCgetFrameB, with_custodian_and_ward<1, 2>()),          DESCsetFrameB)
        .add_property("linearLimitMin",     make_function(DESCgetLinearLimitMin, return_value_policy<return_by_value>()),  DESCsetLinearLimitMin)
        .add_property("linearLimitMax",     make_function(DESCgetLinearLimitMax, return_value_policy<return_by_value>()),  DESCsetLinearLimitMax)
        .add_property("angularLimitMin",    make_function(DESCgetAngularLimitMin, return_value_policy<return_by_value>()), DESCsetAngularLimitMin)
        .add_property("angularLimitMax",    make_function(DESCgetAngularLimitMax, return_value_policy<return_by_value>()), DESCsetAngularLimitMax);
